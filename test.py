...
def load_data():
    if os.path.exists(DB_FILE): 
        ...
        # [해결] '시장' 컬럼이 없으면 자동으로 생성하여 KeyError 방지
...
if not st.session_state.portfolio.empty:
    with st.spinner('실시간 시세 동기화 중...'):
        for idx, row in st.session_state.portfolio.iterrows():
            ticker = str(row['종목코드'])
            market = row.get('시장', '국내') # 안전하게 시장 정보 가져오기
            try:
...
with st.expander("➕ 종목 추가 및 수정", expanded=(st.session_state.edit_index is not None)):
    ...
    with n_col:
        if add_market == "국내":
            ...
            add_name = st.text_input("해외 티커 (예: AAPL)")
...
with c_btm2:
    st.subheader("💵 현금 관리")
    nc = st.number_input("보유 예수금(원)", value=curr_cash, step=10000.0)
