        ...
        for idx, row in st.session_state.portfolio.iterrows():
            ...
            try:
                ...
                if not df_h.empty:
                    ...
                    # 환율 반영 수익 계산
                    buy_krw = buy_amt * (exchange_rate if market == '해외' else 1)
                    val_krw = val_amt * (exchange_rate if market == '해외' else 1)
...
for m_label, m_key in [("🇰🇷 국내 주식", "국내"), ("🇺🇸 미국 주식", "해외")]:
    ...
    if m_list:
        ...
        titles = ["종목명", "고점대비", "평단가", "수량", "평가금액", "현재가", "수익(률)", "신호", "", ""]
        ...
        for item in m_list:
            ...
            if d[8].button("📝", key=f"e_{item['idx']}"): st.session_state.edit_index = item['idx']; st.rerun()
...
# B. 종목 추가/수정 (검색 보완 핵심)
with st.expander("➕ 종목 추가 및 수정", expanded=(st.session_state.edit_index is not None)):
    m_col, n_col, p_col, q_col, t_col = st.columns(5)
    
    with m_col: add_market = st.radio("시장", ["국내", "해외"], horizontal=True)
    with n_col:
        if add_market == "국내":
            add_name = st.selectbox("종목 선택", options=[""] + stock_names)
            final_ticker = stock_dict.get(add_name, "")
        else:
            add_name = st.text_input("해외 종목명/티커 (예: AAPL)")
            final_ticker = add_name.upper()
            
    with p_col: add_price = st.number_input("평단가", min_value=0.0)
    with q_col: add_qty = st.number_input("수량", min_value=0)
