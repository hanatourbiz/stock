...
# 커스텀 CSS: 로직은 건드리지 않고 모바일 가로 스크롤 및 버튼 가독성만 개선
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@100;400;700&display=swap');
    html, body, [class*="css"] { font-family: 'Noto+Sans+KR', sans-serif; }
    
    /* [핵심] 모바일 겹침 방지: 전체 리스트 영역에 가로 스크롤 부여 */
    .stHorizontalBlock {
        overflow-x: auto;
        white-space: nowrap;
    ...
    /* 각 컬럼의 최소 너비를 확보하여 모바일에서 글자가 겹치지 않게 함 */
    [data-testid="column"] {
        min-width: 90px;
