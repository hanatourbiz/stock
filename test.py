...
with c_btm2:
    ...
    if st.button("현금 잔액 업데이트"):
        save_cash(nc); st.rerun()import streamlit as st
import FinanceDataReader as fdr
import yfinance as yf
import pandas as pd
import os
...
# 커스텀 CSS: 자산 요약 블록 크기 및 세로 중앙 정렬 보정
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@100;400;700&display=swap');
    html, body, [class*="css"] { font-family: 'Noto+Sans+KR', sans-serif; }
...
if portfolio_details:
    ...
    for item in portfolio_details:
        ...
        # 각 컬럼 내부에서 세로 중앙 정렬을 위해 st.container 사용 또는 공백 활용
        d = st.columns([1.5, 1.2, 0.8, 0.5, 1.2, 1.2, 1.2, 1.0, 0.4, 0.4])
        
        # 종목명 (중앙 정렬 느낌을 위해 상단 공백 살짝 추가)
        d[0].markdown(f"<div style='padding-top:10px;'><b>{r['종목명']}</b></div>", unsafe_allow_html=True)
        d[1].markdown(f"<span style='font-size:0.85em;'>{r['기준일']}<br>(高:{mx:,.0f})</span>", unsafe_allow_html=True)
        d[2].markdown(f"<div style='padding-top:10px;'>{r['평균매수가']:,.0f}</div>", unsafe_allow_html=True)
        d[3].markdown(f"<div style='padding-top:10px;'>{r['주식수']}</div>", unsafe_allow_html=True)
        d[4].markdown(f"<div style='padding-top:10px;'><b>{item['val_amt']:,.0f}원</b></div>", unsafe_allow_html=True)
        ...
        d[7].markdown(f"<div style='margin-top:10px; background-color:{bg}; color:{clr}; padding:4px 8px; border-radius:15px; text-align:center; font-weight:bold; font-size:0.7em;'>{sig}</div>", unsafe_allow_html=True)
        
        # 버튼들도 높이 조절
        with d[8]: st.markdown("<div style='padding-top:5px;'></div>", unsafe_allow_html=True); st.button("📝", key=f"e_{item['idx']}", on_click=lambda idx=item['idx']: exec(f"st.session_state.edit_index = {idx}"))
        with d[9]: st.markdown("<div style='padding-top:5px;'></div>", unsafe_allow_html=True); st.button("🗑️", key=f"d_{item['idx']}", on_click=lambda idx=item['idx']: (st.session_state.portfolio.drop(idx).to_csv(DB_FILE, index=False)))
